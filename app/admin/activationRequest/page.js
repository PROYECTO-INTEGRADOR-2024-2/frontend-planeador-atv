  ...
  const aceptarSolicitud = async (studentId, aplicationId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8081/api/v1/persons/activateTutor/${studentId}/${aplicationId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.success(" Solicitud aceptada - Estudiante activado como tutor");
    } catch (err) {
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
                        ...
                        <td className="border border-gray-200 px-4 py-3">
                          <div className="flex gap-2 justify-center">
                            <button
                              ...
                              disabled={
                                procesando[solicitud.studentId] ||
                                solicitud.requestState !== "pendiente"
                              }
                              className="flex items-center gap-1 px-3 py-1 bg-green-600 text-white rounded text-sm hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                              <Check className="w-4 h-4" />
                            ...
                            <button
                              onClick={() =>
                                rechazarSolicitud(solicitud.requestId)
                              }
                              disabled={
                                procesando[solicitud.studentId] ||
                                solicitud.requestState !== "pendiente"
            ...
            <div className="mt-6 text-sm text-gray-600">
              ...
              <p>
                Pendientes:{" "}
                {
                  solicitudes.filter((s) => s.requestState === "pendiente")
                    .length
                }
              </p>
            </div>
