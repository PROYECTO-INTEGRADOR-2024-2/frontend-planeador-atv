...
const activationRequest = () => {
    ...
    const aceptarSolicitud = async (studentId, aplicationId) => {
        ...
        try {
            const response = await fetch(`http://localhost:8081/api/v1/persons/activateTutor/${studentId}/${aplicationId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const rechazarSolicitud = async (applicationId) => {
        ...
        try {
            const response = await fetch(`http://localhost:8081/api/v1/application/reject/${applicationId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            alert(' Solicitud rechazada - Notificar al estudiante')
        } catch (err) {
            alert(`Error al aceptar la solicitud: ${err.message}`)
        } finally {
            setProcesando(prev => ({ ...prev, [studentId]: false }));
